<?php
// crypto/SHA256.php
declare(strict_types=1);

final class SHA256
{
    private const K = [
        0x428a2f98,0x71374491,0xb5c0fbcf,0xe9b5dba5,0x3956c25b,0x59f111f1,0x923f82a4,0xab1c5ed5,
        0xd807aa98,0x12835b01,0x243185be,0x550c7dc3,0x72be5d74,0x80deb1fe,0x9bdc06a7,0xc19bf174,
        0xe49b69c1,0xefbe4786,0x0fc19dc6,0x240ca1cc,0x2de92c6f,0x4a7484aa,0x5cb0a9dc,0x76f988da,
        0x983e5152,0xa831c66d,0xb00327c8,0xbf597fc7,0xc6e00bf3,0xd5a79147,0x06ca6351,0x14292967,
        0x27b70a85,0x2e1b2138,0x4d2c6dfc,0x53380d13,0x650a7354,0x766a0abb,0x81c2c92e,0x92722c85,
        0xa2bfe8a1,0xa81a664b,0xc24b8b70,0xc76c51a3,0xd192e819,0xd6990624,0xf40e3585,0x106aa070,
        0x19a4c116,0x1e376c08,0x2748774c,0x34b0bcb5,0x391c0cb3,0x4ed8aa4a,0x5b9cca4f,0x682e6ff3,
        0x748f82ee,0x78a5636f,0x84c87814,0x8cc70208,0x90befffa,0xa4506ceb,0xbef9a3f7,0xc67178f2
    ];

    public static function hash(string $msg, bool $rawOutput = true): string
    {
        $h0=0x6a09e667; $h1=0xbb67ae85; $h2=0x3c6ef372; $h3=0xa54ff53a;
        $h4=0x510e527f; $h5=0x9b05688c; $h6=0x1f83d9ab; $h7=0x5be0cd19;

        $ml = strlen($msg) * 8;

        // padding
        $msg .= "\x80";
        while ((strlen($msg) % 64) !== 56) $msg .= "\x00";

        // append length (64-bit big endian)
        $hi = intdiv($ml, 0x100000000);
        $lo = $ml & 0xffffffff;
        $msg .= pack("N2", $hi, $lo);

        $chunks = str_split($msg, 64);
        foreach ($chunks as $chunk) {
            $w = array_values(unpack("N16", $chunk));
            for ($i=16; $i<64; $i++) {
                $s0 = self::rotr($w[$i-15],7) ^ self::rotr($w[$i-15],18) ^ self::shr($w[$i-15],3);
                $s1 = self::rotr($w[$i-2],17) ^ self::rotr($w[$i-2],19) ^ self::shr($w[$i-2],10);
                $w[$i] = self::add32(self::add32(self::add32($w[$i-16], $s0), $w[$i-7]), $s1);
            }

            $a=$h0; $b=$h1; $c=$h2; $d=$h3; $e=$h4; $f=$h5; $g=$h6; $h=$h7;

            for ($i=0; $i<64; $i++) {
                $S1 = self::rotr($e,6) ^ self::rotr($e,11) ^ self::rotr($e,25);
                $ch = ($e & $f) ^ ((~$e) & $g);
                $temp1 = self::add32(self::add32(self::add32(self::add32($h, $S1), $ch), self::K[$i]), $w[$i]);
                $S0 = self::rotr($a,2) ^ self::rotr($a,13) ^ self::rotr($a,22);
                $maj = ($a & $b) ^ ($a & $c) ^ ($b & $c);
                $temp2 = self::add32($S0, $maj);

                $h=$g; $g=$f; $f=$e;
                $e=self::add32($d, $temp1);
                $d=$c; $c=$b; $b=$a;
                $a=self::add32($temp1, $temp2);
            }

            $h0 = self::add32($h0,$a);
            $h1 = self::add32($h1,$b);
            $h2 = self::add32($h2,$c);
            $h3 = self::add32($h3,$d);
            $h4 = self::add32($h4,$e);
            $h5 = self::add32($h5,$f);
            $h6 = self::add32($h6,$g);
            $h7 = self::add32($h7,$h);
        }

        $out = pack("N8", $h0,$h1,$h2,$h3,$h4,$h5,$h6,$h7);
        return $rawOutput ? $out : bin2hex($out);
    }

    private static function rotr(int $x, int $n): int
    {
        return (($x >> $n) | ($x << (32-$n))) & 0xffffffff;
    }
    private static function shr(int $x, int $n): int
    {
        return ($x >> $n) & 0xffffffff;
    }
    private static function add32(int $a, int $b): int
    {
        return ($a + $b) & 0xffffffff;
    }
}
